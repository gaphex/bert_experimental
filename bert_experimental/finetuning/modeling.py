...
def embedding_postprocessor(input_tensor,
  ...
  if input_state is not None:
    state_embs, tok_embs = output[:, 0], output[:, 1:]
...
def build_module_fn(config_path, vocab_path, do_lower_case=True, seq_layer=-1, tok_layer=-4):
    ...
    def bert_module_fn(is_training):
        ...
        #tiny_output = tf.convert_to_tensor(model.get_all_encoder_layers())
...
def build_bert_module(config_path, vocab_path, ckpt_path, out_path, 
                      seq_layer=-1, tok_layer=-4, do_lower_case=True):
    ...
    module_fn = build_module_fn(config_path, vocab_path, seq_layer=seq_layer, tok_layer=tok_layer, do_lower_case=do_lower_case)
